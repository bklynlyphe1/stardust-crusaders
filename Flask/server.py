...
df = pd.read_csv("/jojolion/static/train.csv")
app = Flask(__name__)
...
def reverse():
    ...
    reverse.to_csv("/jojolion/static/reverse.csv")
    reverse = pd.read_csv("/jojolion/static/reverse.csv", index_col=0)
    reverse = reverse.head()
    odd_cols = df[df.columns[::2]]
    odd_cols.to_csv("/jojolion/static/odd_cols.csv")
    odd_cols = pd.read_csv("/jojolion/static/odd_cols.csv", index_col=0)
...
def msft():
    ...
    if (r.status_code == 200):
        ...
        msft = msft.transpose()
        msft.to_excel("/jojolion/static/msft_dailyquotes.xlsx")
        msft = msft.head()
